...
class TestCodeCheckLanguageEcho:
    """v1.27 round 3: language field is body-controlled and was echoed unsanitized."""

    def test_secrets_language_echo_sanitized(self):
        evil = "python\r\nINJECT<script>‮"
        r = client.post("/v1/check/secrets", json={"code": "x = 1", "language": evil})
        assert r.status_code == 400
    ...
    def test_injection_language_echo_sanitized(self):
        evil = "python\r\nINJECT<script>‮"
        r = client.post("/v1/check/injection", json={"code": "x = 1", "language": evil})
        assert r.status_code == 400
        detail = r.json()["error"]["message"]
        assert "\r" not in detail
        assert "\n" not in detail
...
class TestDependenciesRoute:
    ...
    def test_findings_package_echo_sanitized(self):
        """Pre-existing surface fixed alongside v1.27 CRITICAL: package name echoed in
        findings[].package + remediation must not carry CRLF / Trojan-Source / HTML payloads."""
        ...
        # _normalize_product lowercases and strips, but the raw pkg.name is what gets echoed.
        # Stub the CVE search to always return one match for any product name.
        async def stub_search(product_names, limit_per_product=20):
            return {pn.strip().lower(): [cve_match] for pn in product_names}

        with patch("codesec.routes.asearch_cves_by_products_bulk", side_effect=stub_search):
            r = client.post("/v1/check/dependencies", json={"packages": [{"name": evil_name, "version": "1.0"}]})
        assert r.status_code == 200
        finding = r.json()["findings"][0]
        assert "\r" not in finding["package"]
        assert "\n" not in finding["package"]
        assert "<" not in finding["package"]
        assert "‮" not in finding["package"]
    ...
    def test_partial_fill_when_quota_low(self):
        """v1.27: 11 packages with only 4 quota units left → 5 processed, 6 skipped (no 422)."""
        ...
        with patch("auth.aauthenticate", new_callable=AsyncMock, return_value=auth_ctx):
            ...
            assert len(data["skipped_due_to_rate_limit"]) == 6
            assert data["skipped_due_to_rate_limit"] == [f"pkg{i}" for i in range(5, 11)]
    ...
    def test_consume_bulk_called_with_extra(self):
        """Verify per-package charging: N packages → aconsume_bulk(extra=processable-1) after require_auth's 1."""
        from unittest.mock import AsyncMock, patch
        ...
        with (
            ...
            assert mock_consume.call_args.args[2] == 4  # extra = processable(5) - 1

    def test_bulk_rate_limit_race_falls_back_to_one(self):
...
class TestScanHeadersPivotHints:
    """Batch 2: scan_headers always emits 3 hints (ssl_check, tech_fingerprint, seo_audit)."""

    def test_scan_headers_pivot_emits_three_fixed_hints(self):
        from codesec.routes import _scan_headers_pivot_hints

        record = {"domain": "example.com", "score": 80, "grade": "B"}
        hints = _scan_headers_pivot_hints(record)
        tools = [h.tool for h in hints]
        assert tools == ["ssl_check", "tech_fingerprint", "seo_audit"]
        for h in hints:
