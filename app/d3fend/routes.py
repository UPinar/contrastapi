...
def _d3fend_defense_search_pivot_hints(rows: list[dict]) -> list[PivotHint]:
    """Top-5 D3FEND defense iteration: d3fend_defense_lookup per item only (no cross-link — search shape lacks attack_techniques)."""
    if not rows:
        return []
    hints: list[PivotHint] = []
    for row in rows[:5]:
        did = row.get("defense_id") or ""
        if not did:
            continue
        hints.append(
            PivotHint(
                tool="d3fend_defense_lookup",
                input=did,
                reason="Full D3FEND defense record: tactic, artifact, ATT&CK technique mappings.",
            )
        )
    return hints
...
async def d3fend_defense_search(
    ...
    next_calls = _d3fend_defense_search_pivot_hints(rows)
