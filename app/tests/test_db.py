...
def test_init_all_dbs_creates_tables():
    from db import get_api_db, get_cve_db, get_cache_db
...
def test_touch_api_key():
    from db import save_api_key, touch_api_key, get_api_key
...
def test_deactivate_api_key():
    from db import save_api_key, deactivate_api_key, get_api_key
...
def test_cache_save_and_get():
    from db import save_cached_domain, get_cached_domain
...
def test_cache_expired(monkeypatch):
    from db import save_cached_domain, get_cached_domain, get_cache_db
    save_cached_domain("expired.com", {"data": True})
    # Manually set fetched_at to 25 hours ago
    old_time = (datetime.now(timezone.utc) - timedelta(hours=25)).isoformat()
    with get_cache_db() as con:
        con.execute("UPDATE domain_cache SET fetched_at = ? WHERE domain = ?",
                     (old_time, "expired.com"))
...
def test_upsert_and_get_cve():
    from db import upsert_cve, get_cve
    cve = {
...
def test_search_cves_by_severity():
    from db import upsert_cve, search_cves
...
def test_get_kev_cves():
    from db import upsert_cve, get_kev_cves
...
def test_get_epss():
    from db import upsert_cve, get_epss
...
def test_sync_status():
    from db import update_sync_status, get_sync_status
...
def test_get_recent_cves():
    from db import upsert_cve, get_recent_cves
...
def test_maintenance_runs():
    from db import maintenance, log_usage
...
class TestLogUsageAssertions:
    def test_records_inserted(self):
        from db import log_usage, get_api_db
        log_usage("1.2.3.4", "/v1/cve/test")
        log_usage("1.2.3.4", "/v1/cve/test2", key_hash="keyhash1")
        with get_api_db() as con:
...
class TestMaintenancePurge:
    def test_purges_old_usage(self):
        from datetime import datetime, timezone, timedelta
        from db import maintenance, log_usage, get_api_db
