...
def _create_nowpayments_invoice(api_key: str, order_id: str) -> dict:
    ...
    pick from any coin enabled on the account. The $15/mo price clears
    NOWPayments per-coin minimums (USDT TRC20 ~12, USDT ERC20 ~14, BTC/ETH
    lower fiat-equivalent) so popular stablecoins are usable, and all settle
    to the USDT TRC20 payout wallet.
...
async def crypto_checkout(request: Request) -> dict:
    """Create a NOWPayments invoice for the $15/mo Pro tier.
