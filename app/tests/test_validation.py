...
# --- v1.27 sanitize_echo (bulk skipped_due_to_rate_limit echo defense) ---


class TestSanitizeEcho:
    def test_strips_crlf(self):
        from validation import sanitize_echo

        assert sanitize_echo("8.8.8.8\r\nINJECTED") == "8.8.8.8INJECTED"

    def test_strips_null_byte(self):
        from validation import sanitize_echo

        assert sanitize_echo("evil\x00.com") == "evil.com"

    def test_strips_bidi_override(self):
        from validation import sanitize_echo

        # U+202E RIGHT-TO-LEFT OVERRIDE — Trojan-Source vector
        assert sanitize_echo("good‮rab.exe") == "goodrab.exe"

    def test_strips_html_chars(self):
        from validation import sanitize_echo

        assert sanitize_echo("<script>alert(1)</script>") == "scriptalert(1)/script"

    def test_strips_ansi_escape(self):
        from validation import sanitize_echo

        # ESC sequence — terminal renderers may interpret
        assert sanitize_echo("a\x1b[31mRED\x1b[0mb") == "a[31mRED[0mb"

    def test_caps_length(self):
        from validation import sanitize_echo

        assert len(sanitize_echo("a" * 1024)) == 256

    def test_passes_through_normal_input(self):
        from validation import sanitize_echo

        assert sanitize_echo("CVE-2024-3094") == "CVE-2024-3094"
        assert sanitize_echo("AML.T0000.001") == "AML.T0000.001"
        assert sanitize_echo("8.8.8.8") == "8.8.8.8"
        assert sanitize_echo("example.com") == "example.com"

    def test_non_string_input(self):
        from validation import sanitize_echo

        assert sanitize_echo(None) == ""  # type: ignore[arg-type]
        assert sanitize_echo(42) == ""  # type: ignore[arg-type]
...
# --- B4a v1.30.0 ALLOWED_UPSTREAM_HOSTS ---


class TestAssertUpstreamHostAllowed:
    """Defense-in-depth allowlist for upstream API URLs (NVD/EPSS/KEV/MITRE/...)."""

    def test_allows_known_upstream_host(self):
        from validation import assert_upstream_host_allowed

        assert_upstream_host_allowed("https://services.nvd.nist.gov/rest/json/cves/2.0")
        assert_upstream_host_allowed("https://api.first.org/data/v1/epss")
        assert_upstream_host_allowed("https://api.github.com/advisories")
    ...
    def test_rejects_unknown_host(self):
        ...
        with pytest.raises(ValueError, match="not in allowlist"):
            assert_upstream_host_allowed("https://evil.example.com/payload")
    ...
    def test_rejects_non_http_scheme(self):
        ...
        with pytest.raises(ValueError, match="non-HTTP scheme"):
            assert_upstream_host_allowed("file:///etc/passwd")
        with pytest.raises(ValueError, match="non-HTTP scheme"):
            assert_upstream_host_allowed("gopher://api.github.com/")
    ...
    def test_rejects_missing_host(self):
        ...
        with pytest.raises(ValueError, match="missing a hostname"):
            assert_upstream_host_allowed("https://")

    def test_host_match_is_case_insensitive(self):
        from validation import assert_upstream_host_allowed

        # DNS is case-insensitive but an attacker shouldn't bypass via SERVICES.NVD.NIST.GOV.
        assert_upstream_host_allowed("https://SERVICES.NVD.NIST.GOV/rest/json/cves/2.0")
