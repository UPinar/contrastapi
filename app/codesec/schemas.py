...
class DependenciesResponse(BaseSuccessResponse):
    ...
    total: int = Field(
        default=0, description="Total CVE findings across processed packages (not the input package count)."
    )
    processed: int = Field(
        default=0,
    ...
    skipped_due_to_rate_limit: list[str] = Field(
        default_factory=list,
        description=(
            "Package names that were not scanned because the caller's remaining hourly quota "
