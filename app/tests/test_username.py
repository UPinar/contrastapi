...
class TestUsernameLookupUnit:
    def test_empty_username(self):
        ...
        result = asyncio.run(username_lookup(""))
    ...
    def test_too_long(self):
        ...
        result = asyncio.run(username_lookup("a" * 100))
        assert "error" in result
    ...
    def test_path_traversal_blocked(self):
        ...
        result = asyncio.run(username_lookup("../etc/passwd"))
        assert "error" in result

    def test_null_byte_blocked(self):
        from domain.username import username_lookup

        result = asyncio.run(username_lookup("user\x00"))
        assert "error" in result

    def test_invalid_chars(self):
        from domain.username import username_lookup

        result = asyncio.run(username_lookup("user name!"))
        assert "error" in result
    ...
    def test_at_prefix_stripped(self):
        ...
        with patch("domain.username._client", new_callable=AsyncMock) as mock_client:
            mock_client.head.return_value = resp_404
            mock_client.get.return_value = resp_404
            result = asyncio.run(username_lookup("@testuser"))
    ...
    def test_case_preserved(self):
        ...
        with patch("domain.username._client", new_callable=AsyncMock) as mock_client:
            mock_client.head.return_value = resp_404
            mock_client.get.return_value = resp_404
            result = asyncio.run(username_lookup("MyUser"))
            assert result["username"] == "MyUser"

    @patch("domain.username._client", new_callable=AsyncMock)
    ...
    @patch("domain.username.asyncio.sleep", new_callable=AsyncMock)
    @patch("domain.username._client", new_callable=AsyncMock)
    def test_403_persistent_marked_blocked(self, mock_client, mock_sleep):
        ...
        result = asyncio.run(_check_platform("twitter", "https://x.com/test", "https://x.com/test", "head", None))
    ...
    def test_inverted_indicator_found(self, mock_client):
        ...
        result = asyncio.run(_check_platform("telegram", "https://t.me/x", "https://t.me/x", "get", "!tgme_page_extra"))
        assert result["status"] == "found"
    ...
    def test_summary_format_found(self, mock_client):
        ...
        result = asyncio.run(username_lookup("torvalds"))
    ...
    def test_summary_format_not_found(self, mock_client):
        ...
        result = asyncio.run(username_lookup("xyznonexistent999"))
    ...
    def test_found_count_matches(self, mock_client):
        ...
        result = asyncio.run(username_lookup("allplatforms"))
...
class TestUsernameRetryAndVerdict:
    ...
    def test_ua_rotates_between_retries(self, mock_client, mock_sleep):
        ...
        asyncio.run(_check_platform("github", "https://github.com/test", "https://github.com/test", "head", None))
    ...
    def test_npm_gets_referer_header(self, mock_client, mock_sleep):
        ...
        asyncio.run(_check_platform("npm", "https://www.npmjs.com/~test", "https://www.npmjs.com/~test", "head", None))
    ...
    def test_verdict_complete_when_all_resolve(self, mock_client, mock_sleep):
        ...
        result = asyncio.run(username_lookup("neverexistsuser999xyz"))
