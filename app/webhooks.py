...
def _handle_subscription_ended(data: dict, event_name: str) -> dict:
    ...
    # Log only literal state — no value from `data` (webhook payload) flows into
    # the log call. CodeQL's `py/clear-text-logging-sensitive-data` taint tracker
    # marks anything derived from the webhook dict as sensitive (including the
    # SQL rowcount, transitively), so we emit one of two fixed strings here and
    # let the response body / Telegram notify carry the actual values.
    if count:
        logger.info("Webhook subscription ended: deactivation succeeded")
    else:
        logger.warning("Webhook subscription ended: no matching key (idempotent)")
