...
class TestBulkIocLookup:
    ...
    def test_bulk_ioc_skipped_indicators_are_sanitized(self, mock_detect, mock_tf, mock_feodo, mock_urlhaus, client):
        """v1.27 CRITICAL fix: skipped_due_to_rate_limit must not echo CRLF / Trojan-Source / HTML."""
        auth_ctx = AuthCtx(
            tier="free",
            key_hash=None,
            client_ip="127.0.0.1",
            ratelimit_limit=100,
            ratelimit_remaining=0,  # require_auth paid 1; only 1 indicator processable
            ratelimit_reset=0,
            ratelimit_cost=1,
        )
        # First indicator is processable; the next two carry CRLF and bidi overrides.
        # The endpoint must echo SANITIZED versions in skipped_due_to_rate_limit.
        indicators = ["8.8.8.8", "8.8.8.9\r\nINJECTED", "evil‮rab.exe"]
        with patch("auth.aauthenticate", new_callable=AsyncMock, return_value=auth_ctx):
            r = client.post("/v1/iocs/bulk", json={"indicators": indicators})
        assert r.status_code == 200
        data = r.json()
        skipped = data["skipped_due_to_rate_limit"]
    ...
    @patch("ioc.routes.query_threatfox", new_callable=AsyncMock, return_value={"found": False})
    @patch("ioc.routes.detect_indicator_type", return_value="ip")
    def test_bulk_ioc_partial_fill_when_quota_low(self, mock_detect, mock_tf, mock_feodo, mock_urlhaus, client):
        """v1.27: when remaining quota < input list, surplus lands in skipped_due_to_rate_limit."""
        # Free tier with only 1 quota unit left → can process 2 indicators total.
        auth_ctx = AuthCtx(
            tier="free",
            key_hash=None,
            client_ip="127.0.0.1",
        ...
        indicators = [f"8.8.8.{i}" for i in range(7)]
        with patch("auth.aauthenticate", new_callable=AsyncMock, return_value=auth_ctx):
            r = client.post("/v1/iocs/bulk", json={"indicators": indicators})
        assert r.status_code == 200
        data = r.json()
        ...
        assert data["skipped_due_to_rate_limit"] == indicators[2:]
    ...
    def test_bulk_ioc_response_exposes_invalid_count(self, client):
        ...
        successful + failed + timed_out + invalid == processed (v1.27: total > processed
        when partial-fill from rate-limit kicks in, so invariant tracks processed)."""
        ...
        assert data["successful"] + data["failed"] + data["timed_out"] + data["invalid"] == data["processed"]
    ...
    def test_bulk_ioc_consume_bulk_race_falls_back_to_one(self, mock_auth, mock_consume, client):
    ...
    def test_bulk_ioc_consume_bulk_call_args(self, mock_auth, mock_consume, client):
        """Verify aconsume_bulk receives extra = processable - 1 (authenticate consumed 1)."""
        r = client.post("/v1/iocs/bulk", json={"indicators": [f"8.8.8.{i}" for i in range(5)]})
        # v1.27: race fallback returns 200 with partial-fill (was 429 pre-v1.27)
        assert r.status_code == 200
        mock_consume.assert_called_once()
        args = mock_consume.call_args.args
        assert args[0] == "api"
        assert args[2] == 4  # extra = processable(5) - 1 = 4


class TestPhishingCheckPivotHints:
    """Batch 3: phishing_check conditional next_calls (threat_level + is_malicious)."""

    def test_phishing_check_clean_emits_tech_fingerprint(self):
        from ioc.routes import _phishing_check_pivot_hints

        record = {"host": "example.com", "is_malicious": False, "threat_level": "none"}
        hints = _phishing_check_pivot_hints(record)
        tools = [h.tool for h in hints]
        assert tools == ["tech_fingerprint"]
        for h in hints:
            assert h.input == "example.com"
            assert h.reason

    def test_phishing_check_high_threat_emits_domain_report_and_threat_intel(self):
        from ioc.routes import _phishing_check_pivot_hints

        record = {"host": "bad.example", "is_malicious": True, "threat_level": "high"}
        hints = _phishing_check_pivot_hints(record)
        tools = [h.tool for h in hints]
        assert tools == ["domain_report", "threat_intel"]

    def test_phishing_check_medium_threat_emits_domain_report_only(self):
        from ioc.routes import _phishing_check_pivot_hints

        record = {"host": "sus.example", "is_malicious": True, "threat_level": "medium"}
        hints = _phishing_check_pivot_hints(record)
        tools = [h.tool for h in hints]
        assert tools == ["domain_report"]

    def test_phishing_check_no_host_returns_empty(self):
        from ioc.routes import _phishing_check_pivot_hints

        hints = _phishing_check_pivot_hints({"host": "", "is_malicious": False, "threat_level": "none"})
        assert hints == []


class TestHashLookupPivotHints:
    """Batch 6: hash_lookup emits ioc_lookup when MalwareBazaar found=True; empty otherwise."""

    def test_hash_lookup_found_emits_ioc_lookup(self):
        from ioc.routes import _hash_lookup_pivot_hints

        record = {"hash": "deadbeef" * 8, "found": True, "malware_family": "Emotet"}
        hints = _hash_lookup_pivot_hints(record)
        assert len(hints) == 1
        assert hints[0].tool == "ioc_lookup"
        assert hints[0].input == "deadbeef" * 8
        assert hints[0].reason

    def test_hash_lookup_not_found_emits_empty(self):
        from ioc.routes import _hash_lookup_pivot_hints

        record = {"hash": "abc123" * 6 + "abcd", "found": False}
        assert _hash_lookup_pivot_hints(record) == []

    def test_hash_lookup_missing_found_emits_empty(self):
        from ioc.routes import _hash_lookup_pivot_hints

        assert _hash_lookup_pivot_hints({"hash": "x" * 32}) == []


class TestBulkIocLookupPerItemPivotHints:
    """Batch 6: bulk_ioc_lookup per-item type-dispatch (hash/ip/domain → drill, url → skip)."""

    def test_bulk_ioc_per_item_hash_emits_hash_lookup(self):
        from ioc.routes import _bulk_ioc_per_item_pivot_hints

        item = {
            "indicator": "deadbeef" * 8,
            "status": "ok",
            "ioc": {"type": "hash", "threat_level": "high", "sources": {}},
        }
        hints = _bulk_ioc_per_item_pivot_hints(item)
        assert len(hints) == 1
        assert hints[0].tool == "hash_lookup"
        assert hints[0].input == "deadbeef" * 8

    def test_bulk_ioc_per_item_ip_emits_ip_lookup(self):
        from ioc.routes import _bulk_ioc_per_item_pivot_hints

        item = {
            "indicator": "8.8.8.8",
            "status": "ok",
            "ioc": {"type": "ip", "threat_level": "none", "sources": {}},
        }
        hints = _bulk_ioc_per_item_pivot_hints(item)
        assert len(hints) == 1
        assert hints[0].tool == "ip_lookup"
        assert hints[0].input == "8.8.8.8"

    def test_bulk_ioc_per_item_domain_emits_domain_report(self):
        from ioc.routes import _bulk_ioc_per_item_pivot_hints

        item = {
            "indicator": "example.com",
            "status": "ok",
            "ioc": {"type": "domain", "threat_level": "low", "sources": {}},
        }
        hints = _bulk_ioc_per_item_pivot_hints(item)
        assert len(hints) == 1
        assert hints[0].tool == "domain_report"
        assert hints[0].input == "example.com"

    def test_bulk_ioc_per_item_url_emits_empty(self):
        from ioc.routes import _bulk_ioc_per_item_pivot_hints

        item = {
            "indicator": "https://example.com/malware.exe",
            "status": "ok",
            "ioc": {"type": "url", "threat_level": "high", "sources": {}},
        }
        assert _bulk_ioc_per_item_pivot_hints(item) == []

    def test_bulk_ioc_per_item_error_status_emits_empty(self):
        from ioc.routes import _bulk_ioc_per_item_pivot_hints
...
class TestBulkIocLookupOuterHints:
    """Batch 6: bulk_ioc_lookup outer envelope = 1 ioc_lookup hint with first ok indicator."""

    def test_bulk_ioc_outer_emits_ioc_lookup_for_first_ok(self):
        from ioc.routes import _bulk_ioc_lookup_outer_hints

        results = [
            {"indicator": "bad", "status": "invalid_format", "ioc": None},
            {"indicator": "8.8.8.8", "status": "ok", "ioc": {"type": "ip"}},
            {"indicator": "example.com", "status": "ok", "ioc": {"type": "domain"}},
        ]
        hints = _bulk_ioc_lookup_outer_hints(results)
        assert len(hints) == 1
        assert hints[0].tool == "ioc_lookup"
        assert hints[0].input == "8.8.8.8"

    def test_bulk_ioc_outer_empty_when_all_failed(self):
        from ioc.routes import _bulk_ioc_lookup_outer_hints

        results = [
            {"indicator": "bad", "status": "invalid_format", "ioc": None},
            {"indicator": "x", "status": "error", "ioc": None},
        ]
        assert _bulk_ioc_lookup_outer_hints(results) == []

    def test_bulk_ioc_outer_empty_on_no_results(self):
        from ioc.routes import _bulk_ioc_lookup_outer_hints

        assert _bulk_ioc_lookup_outer_hints([]) == []
