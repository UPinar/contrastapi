...
class BulkDomainResponse(BaseSuccessResponse):
    ...
    total: int = Field(
        default=0,
        description="Total number of input domains submitted (== processed + len(skipped_due_to_rate_limit)).",
    )
    processed: int = Field(
        default=0,
        description="Count of domains actually scanned (== len(results)). Equal to total unless dynamic-budget partial-fill kicked in.",
    )
    skipped_due_to_rate_limit: list[str] = Field(
        default_factory=list,
        description=(
            "Domains that were not processed because the caller's remaining hourly quota was "
            "smaller than the input list. Empty when full budget was available. Distinct from "
            "per-domain timeouts (those still consume budget and surface as status='error' in results)."
        ),
    )
    ...
    partial: bool = Field(
        default=False, description="True when at least one item failed, timed out, or was skipped due to rate limit."
    )
