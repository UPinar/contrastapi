...
from validation import _is_valid_format, clean_domain, is_private_ip, is_valid_ip, sanitize_echo, validate_domain
...
def _validate_domain_input(raw_domain: str) -> tuple[str, str]:
    ...
    if is_valid_ip(raw_domain):
        # Sanitize for echo even though is_valid_ip predicate already constrains the value —
        # defense-in-depth for the response detail field.
        safe_raw = sanitize_echo(raw_domain)
        raise HTTPException(
            status_code=400,
            detail=f"'{safe_raw}' is an IP address, not a domain. Use /v1/ip/{safe_raw} instead.",
        )
...
def _dns_lookup_pivot_hints(record: dict) -> list[PivotHint]:
    """Always: whois_lookup + subdomain_enum. Conditional: ssl_check (A/AAAA), email_mx (MX)."""
    domain = record["domain"]
    hints: list[PivotHint] = [
        PivotHint(tool="whois_lookup", input=domain, reason="Registration data: registrar, nameservers, expiry."),
        PivotHint(
            tool="subdomain_enum",
            input=domain,
            reason="Map attack surface — enumerate subdomains via crt.sh CT logs + DNS wordlist (passive).",
    ...
    rec = record.get("records") or {}
    if rec.get("a") or rec.get("aaaa"):
        hints.append(
            PivotHint(
                tool="ssl_check",
                input=domain,
                reason="Inspect TLS certificate: grade, protocol, cipher, expiry, AIA chain.",
            )
        )
    if rec.get("mx"):
        hints.append(
            PivotHint(
                tool="email_mx", input=domain, reason="Email security: MX provider, SPF/DMARC/DKIM validation, grade."
            )
        )
    return hints


def _whois_lookup_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 3: dns_lookup + subdomain_enum + ssl_check."""
    domain = record["domain"]
    return [
        PivotHint(tool="dns_lookup", input=domain, reason="DNS records: A, MX, NS, TXT for infrastructure mapping."),
        PivotHint(
            tool="subdomain_enum",
            input=domain,
            reason="Map attack surface — enumerate subdomains via crt.sh CT logs + DNS wordlist (passive).",
...
def _ssl_check_pivot_hints(record: dict, tier: str) -> list[PivotHint]:
    """Always 3: tech_fingerprint + scan_headers + dns_lookup. Pro-only: threat_report."""
    domain = record["domain"]
    hints: list[PivotHint] = [
        PivotHint(
            tool="tech_fingerprint",
        ...
        PivotHint(tool="scan_headers", input=domain, reason="Live HTTP security headers: CSP, HSTS, X-Frame-Options."),
        PivotHint(tool="dns_lookup", input=domain, reason="DNS records: A, MX, NS, TXT for infrastructure mapping."),
    ]
    if tier == "pro":
        hints.append(
            PivotHint(
                tool="threat_report",
                input=domain,
                reason="Comprehensive threat intelligence: Shodan, abuse reports, ports.",
            )
        )
    return hints


def _tech_fingerprint_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 4: scan_headers + ssl_check + seo_audit + robots_txt."""
    domain = record["domain"]
    return [
        PivotHint(tool="scan_headers", input=domain, reason="Live HTTP security headers: CSP, HSTS, X-Frame-Options."),
        PivotHint(
        ...
        PivotHint(tool="seo_audit", input=domain, reason="SEO posture: title, meta, H1, canonical, OG tags, JSON-LD."),
        PivotHint(
            tool="robots_txt", input=domain, reason="robots.txt parsing: sitemaps, user-agent rules, crawl delays."
        ),
    ]


def _robots_txt_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 3: seo_audit + tech_fingerprint + domain_report."""
    domain = record["domain"]
    return [
        PivotHint(tool="seo_audit", input=domain, reason="SEO posture: title, meta, H1, canonical, OG tags, JSON-LD."),
        PivotHint(
...
def _redirect_chain_pivot_hints(record: dict) -> list[PivotHint]:
    """Conditional: ssl_check (HTTPS final) + tech_fingerprint (always when host parses) + phishing_check (hop_count>1)."""
    ...
    if final_url.startswith("https://"):
        hints.append(
            PivotHint(
                tool="ssl_check",
                input=final_host,
                reason="Inspect TLS certificate of final landing: grade, protocol, cipher, expiry.",
            )
        )
    hints.append(
        PivotHint(
            tool="tech_fingerprint",
            input=final_host,
            reason="Detect tech stack of redirect target (CMS, framework, CDN, web server).",
        )
    )
    if (record.get("hop_count") or 0) > 1:
        hints.append(
            PivotHint(
                tool="phishing_check",
                input=record.get("start_url") or final_url,
                reason="Multi-hop redirect chains are a phishing signature — check URLhaus.",
            )
        )
    return hints


def _email_verify_pivot_hints(record: dict) -> list[PivotHint]:
    """Always: email_disposable. Conditional: email_mx (mx_records & not disposable), domain_report (not disposable)."""
    domain = record.get("domain") or ""
    if not domain:
        return []
    hints: list[PivotHint] = [
        PivotHint(
            tool="email_disposable", input=domain, reason="Confirm disposable status with explicit blocklist check."
        ),
    ]
    if not record.get("disposable"):
        if record.get("mx_records"):
            hints.append(
                PivotHint(
                    tool="email_mx", input=domain, reason="Email security audit: MX provider, SPF/DMARC/DKIM, grade."
                )
            )
        hints.append(
            PivotHint(
...
def _brand_assets_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 4: seo_audit + tech_fingerprint + ssl_check + scan_headers."""
    domain = record["domain"]
    return [
        PivotHint(tool="seo_audit", input=domain, reason="SEO posture: title, meta, H1, canonical, OG tags, JSON-LD."),
        PivotHint(
...
def _seo_audit_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 3: robots_txt + tech_fingerprint + scan_headers. Conditional: brand_assets (score<50)."""
    domain = record["domain"]
    hints: list[PivotHint] = [
        PivotHint(
            tool="robots_txt", input=domain, reason="robots.txt parsing: sitemaps, user-agent rules, crawl delays."
...
def _email_mx_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 3: email_verify + email_disposable + dns_lookup."""
    domain = record["domain"]
    return [
        PivotHint(
            tool="email_verify", input=domain, reason="Per-address validation: syntax, role, free-provider check."
        ),
        PivotHint(tool="email_disposable", input=domain, reason="Disposable provider blocklist check on the domain."),
        PivotHint(tool="dns_lookup", input=domain, reason="DNS records: A, MX, NS, TXT for SPF/DMARC context."),
    ]


def _email_disposable_pivot_hints(record: dict) -> list[PivotHint]:
    """Conditional: empty if disposable=true OR no domain. Else: email_mx + email_verify."""
    domain = record.get("domain") or ""
    if not domain or record.get("disposable"):
        return []
    return [
        PivotHint(tool="email_mx", input=domain, reason="Trusted domain — audit MX provider, SPF/DMARC/DKIM grade."),
        PivotHint(
            tool="email_verify", input=domain, reason="Per-address validation: syntax, role, free-provider check."
        ),
    ]
...
def _threat_intel_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 2 (domain_report + ioc_lookup). Conditional: phishing_check when urls_online>0."""
    domain = record.get("domain") or ""
    if not domain:
        return []
    hints: list[PivotHint] = [
        ...
        PivotHint(
            tool="ioc_lookup",
            input=domain,
            reason="Threat-indicator drill: ThreatFox / Feodo Tracker / URLhaus active IOC status.",
        ),
    ]
    if (record.get("urls_online") or 0) > 0:
        hints.append(
            PivotHint(
                tool="phishing_check",
                input=domain,
                reason="Active malicious URLs listed — check for phishing/redirect signature.",
            )
        )
    return hints


def _audit_domain_pivot_hints(record: dict, domain: str) -> list[PivotHint]:
    """Audit bundles tech_fingerprint + live_headers inline.

    Emits subdomain_enum + ssl_check (has_a) + domain_report + scan_headers when
    DNS records present; returns empty list on NXDOMAIN.
    """
    audit_hints: list[PivotHint] = []
    dns_block = record.get("dns") or {}
    has_a = bool(dns_block.get("a") or dns_block.get("aaaa"))
    if dns_block or has_a:
        ...
        audit_hints.append(
            PivotHint(
                tool="scan_headers",
                input=domain,
                reason="Live HTTP security headers: CSP, HSTS, X-Frame-Options, Permissions-Policy.",
            )
        )
    return audit_hints
...
async def dns_records(
    ...
    if cached:
        response = {"domain": domain, "records": cached, "summary": _dns_summary(cached, domain)}
        response["next_calls"] = [h.model_dump() for h in _dns_lookup_pivot_hints(response)]
        return response
    ...
    response = {"domain": domain, "records": records, "summary": _dns_summary(records, domain)}
    response["next_calls"] = [h.model_dump() for h in _dns_lookup_pivot_hints(response)]
    return response
...
async def email_disposable(
    ...
    if cached:
        ...
        next_calls = [h.model_dump() for h in _email_disposable_pivot_hints(cached)]
        return {**cached, "email": email, "summary": summary, "next_calls": next_calls}
    ...
    result["next_calls"] = [h.model_dump() for h in _email_disposable_pivot_hints(result)]
...
async def email_verify_endpoint(
    ...
    if cached:
        ...
        response["next_calls"] = [h.model_dump() for h in _email_verify_pivot_hints(response)]
        return response
...
async def robots_txt_endpoint(
    ...
    result["next_calls"] = [h.model_dump() for h in _robots_txt_pivot_hints(result)]
    await asave_cached_domain(cache_key, result)
    return result
...
async def redirect_chain_endpoint(
    ...
    result["next_calls"] = [h.model_dump() for h in _redirect_chain_pivot_hints(result)]
    await asave_cached_domain(cache_key, result)
    return result
...
async def brand_assets_endpoint(
    ...
    result["next_calls"] = [h.model_dump() for h in _brand_assets_pivot_hints(result)]
...
async def ssl_certificate(
    ...
    # next_calls is generated per-request (tier-aware) and never stored in cache,
    # to avoid Free-tier hint shape leaking into Pro-tier responses (and vice versa).
    cached = await aget_cached_domain(f"ssl:{domain}")
    if cached:
        response = {**cached}
        response["next_calls"] = [h.model_dump() for h in _ssl_check_pivot_hints(response, auth.tier)]
        return response
...
async def threat_intel(
    ...
    response = {
        ...
        **result,
        "summary": summary,
        "verdict": _threat_verdict(unavailable=urlhaus_unavailable),
    }
    response["next_calls"] = [h.model_dump() for h in _threat_intel_pivot_hints(response)]
    return response
...
async def ip_lookup(
    ...
    if not is_valid_ip(ip):
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Invalid IP address: '{sanitize_echo(ip)}'. Expected IPv4 (e.g., 8.8.8.8) or IPv6.",
        )
    if is_private_ip(ip):
        raise HTTPException(
            status_code=400,
            detail=f"Private/reserved IP rejected: {sanitize_echo(ip)}. Public IPs only.",
        )
    client_ip = auth.client_ip
...
async def tech_fingerprint(
    ...
    response = {"domain": domain, **result}
    response["next_calls"] = [h.model_dump() for h in _tech_fingerprint_pivot_hints(response)]
    return response
...
async def asn_lookup(
    ...
    if is_valid_ip(target):
        if is_private_ip(target):
            raise HTTPException(
                status_code=400,
                detail=f"Private/reserved IP rejected: {sanitize_echo(target)}. Public IPs only.",
            )
        ip = target
    else:
        ...
        if not a_records:
            raise HTTPException(
                status_code=422,
                detail=f"Could not resolve domain '{sanitize_echo(target)}' to an IP address",
            )
        ip = a_records[0]
        resolved_ip = ip
...
class _BulkRequest(BaseModel):
    domains: list[str] = Field(
        ...
        description=(
            ...
            "consumes 1 unit of the per-hour quota; entries beyond the caller's remaining quota land "
            "in `skipped_due_to_rate_limit` (v1.27 dynamic budget)."
        ),
    )
...
async def _run_single_report(raw_domain: str, client_ip: str, tier: str = "free") -> dict:
    ...
    if not domain or not resolved_ip:
        # raw_domain is unsanitized request input — strip control chars / HTML before echoing.
        return {
            "domain": sanitize_echo(raw_domain),
            "status": "error",
            "report": None,
...
async def bulk_domain_report(
    ...
    """Bulk domain intelligence — up to 50 domains per call (Pydantic input cap). Each domain
    ...
    # v1.27 dynamic budget: require_auth consumed 1, so the caller can process
    # up to (auth.ratelimit_remaining + 1) domains; the rest become skipped.
    available_budget = auth.ratelimit_remaining + 1
    processable = min(input_count, available_budget)
    extra = processable - 1
    ...
    if extra > 0 and not await ratelimit.aconsume_bulk("api", store_key, extra, limit):
        ...
        # Fall back to processing only what require_auth already paid for.
        processable = 1

    skipped_domains = [sanitize_echo(s) for s in domains[processable:]]
    domains = domains[:processable]
    count = len(domains)
    ...
    if partial:
        summary = f"{successful}/{total} domains scanned (partial — overall timeout reached)"
    elif failed == 0 and timed_out == 0 and skipped_count == 0:
        summary = f"All {total} domains scanned successfully"
    elif successful == 0 and skipped_count == 0:
        summary = f"All {total} domains failed"
    else:
        parts = [f"{successful}/{total} domains scanned"]
        if failed:
    ...
    return {
        ...
        "total": total,
        "processed": count,
        "skipped_due_to_rate_limit": skipped_domains,
        "successful": successful,
        "failed": failed,
        "timed_out": timed_out,
        "partial": partial or skipped_count > 0,
        "summary": summary,
    }
...
async def _audit_domain_impl(
    ...
    *,
    ...
    audit_hints = _audit_domain_pivot_hints(report, domain)
