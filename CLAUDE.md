...
- v1.26.2 extends the same `Accept-Encoding: identity` rule to `domain/ip_intel.py`'s `_intel_client` (cloud / Tor / FireHOL blocklist fetcher) — `_fetch_capped` uses `aiter_bytes()` which has the same decompress-before-yield behavior. Same byte-cap bypass applied via `_intel_client` → same fix
- v1.26.2 `_fetch_capped` rejects negative Content-Length explicitly (CWE-190): `int("-5") > max_bytes` would otherwise evaluate False and let oversize bodies through
- Async I/O (Faz 4 complete as of v1.26.2): all HTTP clients are `httpx.AsyncClient` singletons, lifespan-managed via `aclose()` tuple in `core/lifespan.py`. Domain module is fully async — no `httpx.Client(`, no `run_in_threadpool` for HTTP work, no `asyncio.to_thread` in lifespan. ThreadPoolExecutor only retained for DB sync calls (sqlite3) via `db.py` async wrappers
