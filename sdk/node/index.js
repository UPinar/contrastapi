...
function ContrastAPI(options = {}) {
  ...
  return {
    ...
    cve: {
      ...
      riskScore: (cveId) => get(`/v1/cve/${enc(cveId)}/risk_score`),
      cvssDetails: (vector) => {
        if (typeof vector !== "string" || !vector) {
          throw new Error("vector must be a non-empty CVSS v3.x string");
        }
        return get(`/v1/cvss/details?vector=${encodeURIComponent(vector)}`);
      },
      bulk: (cveIds) => {
