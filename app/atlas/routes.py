...
def _atlas_technique_search_pivot_hints(rows: list[dict]) -> list[PivotHint]:
    """Top-5 ATLAS technique iteration: atlas_technique_lookup per item; +d3fend_defense_for_attack when any top-5 carries attack_reference_id.

    Only the first attack_reference_id seen in top-5 emits a bridge hint (one
    d3fend_defense_for_attack pivot, not N) — keeps response token-cost flat
    and reflects the agent's likely workflow (drill into one bridge, not all).
    """
    if not rows:
        return []
    hints: list[PivotHint] = []
    for row in rows[:5]:
        ...
        hints.append(
            PivotHint(
                tool="atlas_technique_lookup",
                input=tid,
                reason="Full ATLAS technique record: tactics, maturity, ATT&CK bridge mapping.",
            )
        )
    bridge_attack = next(
        (row.get("attack_reference_id") for row in rows[:5] if row.get("attack_reference_id")),
        None,
    )
    if bridge_attack:
        hints.append(
...
def _atlas_case_study_search_pivot_hints(rows: list[dict]) -> list[PivotHint]:
    """Top-5 case study iteration: atlas_case_study_lookup per item; +bulk_atlas_technique_lookup aggregate (capped to 25 unique techniques).

    25-cap is conservative against the bulk endpoint's `_BULK_ATLAS_MAX_IDS=50`
    ceiling: top-5 case studies x ~10 techniques each can hit 50 in worst case,
    and the joined string is rendered in the hint `reason` text - capping at 25
    keeps the response payload bounded while still drilling the most-relevant
    half in one call.
    """
    if not rows:
        return []
    hints: list[PivotHint] = []
    for row in rows[:5]:
        cs_id = row.get("case_study_id") or ""
        if not cs_id:
            continue
        hints.append(
            PivotHint(
                ...
                input=cs_id,
                reason="Full case study record: procedure chain, incident timeline, techniques used.",
            )
        )
    all_techniques: list[str] = []
    for row in rows[:5]:
        for tid in row.get("techniques_used") or []:
            if tid not in all_techniques:
                all_techniques.append(tid)
    if all_techniques:
        capped = all_techniques[:25]
        hints.append(
            PivotHint(
                tool="bulk_atlas_technique_lookup",
                input=",".join(capped),
                reason=(
                    f"Drill into {len(capped)} ATLAS technique(s) referenced across these incidents in a single call."
                ),
            )
        )
    return hints
...
def _bulk_atlas_technique_lookup_outer_hints(results: list[dict]) -> list[PivotHint]:
    """Outer envelope: 1 atlas_case_study_search hint with first successful technique to find real-world incidents."""
    if not results:
        return []
    first_ok = next(
        (item.get("technique_id") for item in results if item.get("status") == "ok"),
        None,
    )
    if not first_ok:
        return []
    return [
        PivotHint(
            tool="atlas_case_study_search",
            input=first_ok,
            reason="Find real-world AI/ML incidents that exercised these techniques.",
        )
    ]
...
async def atlas_technique_search(
    ...
    next_calls = _atlas_technique_search_pivot_hints(rows)
...
async def atlas_case_study_search(
    ...
    next_calls = _atlas_case_study_search_pivot_hints(rows)
...
async def bulk_atlas_technique_lookup(
    ...
    """Bulk ATLAS technique lookup — up to 50 technique ids in one call (Pydantic input cap).
    ...
    # v1.27 dynamic budget: require_auth already consumed 1 unit, so the caller
    # can process up to (auth.ratelimit_remaining + 1) items total; the rest
    # are surfaced as skipped_due_to_rate_limit instead of 429-ing the batch.
    if auth.tier == "pro":
    ...
    if extra > 0 and not await ratelimit.aconsume_bulk("api", store_key, extra, hourly_limit):
        # Race: another worker consumed quota between require_auth and here.
        # Fall back to processing only what require_auth already paid for.
        processable = 1

    skipped_ids = [sanitize_echo(s) for s in ids[processable:]]
    ids = ids[:processable]
    ...
    for tid in ids:
        if not _TECHNIQUE_RE.match(tid):
            safe_tid = sanitize_echo(tid)
            results.append(
                {
                    "technique_id": safe_tid,
                    "status": "invalid_format",
                    "technique": None,
                    "error": (
                        f"Invalid ATLAS technique id format: {safe_tid!r}. Expected 'AML.T####' or 'AML.T####.###'."
                    ),
                }
            )
            continue
        # tid is regex-validated past this point; sanitize defensively for the echo.
        safe_tid = sanitize_echo(tid)
            ...
            results.append(
                {
                    ...
                    "status": "not_found",
                    "technique": None,
                    "error": f"{safe_tid} is not in the MITRE ATLAS catalog",
                }
            )
            continue
    ...
    outer_hints = _bulk_atlas_technique_lookup_outer_hints(results)
    return {
        "results": results,
        "total": total,
        ...
        "summary": f"{successful}/{total} techniques found",
