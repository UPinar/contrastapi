...
from validation import _is_valid_format, clean_domain, is_valid_ip, sanitize_echo, validate_domain
...
async def check_secrets_endpoint(
    ...
    if lang not in ALLOWED_LANGUAGES:
        raise HTTPException(
            status_code=400,
            detail=f"Language '{sanitize_echo(body.language)}' not supported. Allowed: {', '.join(sorted(ALLOWED_LANGUAGES))}",
        )
...
def _scan_headers_pivot_hints(record: dict) -> list[PivotHint]:
    """Always 3: ssl_check + tech_fingerprint + seo_audit."""
...
async def scan_headers_endpoint(
    ...
    response["next_calls"] = [h.model_dump() for h in _scan_headers_pivot_hints(response)]
    return response
...
async def check_dependencies_endpoint(
    ...
    instead of failing the whole batch (v1.27 dynamic budget).
    """
    import ratelimit
    ...
    skipped_packages = [sanitize_echo(p.name) for p in packages[processable:]]
    packages = packages[:processable]
    count = len(packages)
    ...
    for pkg in packages:
        ...
        # Sanitize once per package — pkg.name and pkg.version come from request body.
        safe_pkg_name = sanitize_echo(pkg.name)
        safe_pkg_version = sanitize_echo(pkg.version) if pkg.version else None
        ...
        for cve, fix_version in matched_cves:
            ...
            if fix_version:
                remediation = (
                    f"Upgrade {safe_pkg_name} to {fix_version} or later "
                    f"(current: {safe_pkg_version or 'unknown'}) to patch {cve['cve_id']}"
                )
            else:
                remediation = (
                    f"Check if {safe_pkg_name} {safe_pkg_version or 'current'} is affected by "
                    f"{cve['cve_id']} and upgrade if so"
                )
            findings.append(
                {
                    "package": safe_pkg_name,
                    "version": safe_pkg_version,
                    "cve_id": cve["cve_id"],
                    "severity": severity,
    ...
    if skipped_count:
        summary = f"{summary} — {skipped_count} package{'s' if skipped_count != 1 else ''} skipped (quota)"

    return {
        "findings": findings,
        "total": total,
        "processed": pkg_count,
        "skipped_due_to_rate_limit": skipped_packages,
        "by_severity": by_severity,
        "summary": summary,
    }
