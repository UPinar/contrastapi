...
class TestD3fendDefenseSearchPivotHints:
    """Batch 5b: d3fend_defense_search emits d3fend_defense_lookup per top-5 only (no cross-link — search shape lacks attack_techniques)."""

    def test_d3fend_defense_search_pivot_emits_per_item_lookup(self):
        from d3fend.routes import _d3fend_defense_search_pivot_hints

        rows = [
            {"defense_id": "AccessModeling", "tactic": "Model"},
            {"defense_id": "ContentFiltering", "tactic": "Detect"},
        ]
        hints = _d3fend_defense_search_pivot_hints(rows)
        tools = [h.tool for h in hints]
        assert tools == ["d3fend_defense_lookup", "d3fend_defense_lookup"]
        inputs = [h.input for h in hints]
        assert inputs == ["AccessModeling", "ContentFiltering"]
        for h in hints:
            assert h.reason

    def test_d3fend_defense_search_pivot_caps_at_top_5_results(self):
        from d3fend.routes import _d3fend_defense_search_pivot_hints

        rows = [{"defense_id": f"Defense{i}", "tactic": "Detect"} for i in range(10)]
        hints = _d3fend_defense_search_pivot_hints(rows)
        assert len(hints) == 5
        assert all(h.tool == "d3fend_defense_lookup" for h in hints)

    def test_d3fend_defense_search_pivot_empty_on_no_results(self):
        from d3fend.routes import _d3fend_defense_search_pivot_hints

        assert _d3fend_defense_search_pivot_hints([]) == []
